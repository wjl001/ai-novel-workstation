#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""生成基于 Hermes Agent 多轮对话的图片生成提示词优化产品方案 - 完整版V2（详细版）"""
...
add_bullet(doc, '第一点：提示词结构过于简单。现有提示词仅包含基础描述加通用风格词，缺少结构化的质量控制层、特征锁定层、负面排除层。')
add_bullet(doc, '第二点：缺少迭代优化机制。当前提示词为一次性生成，缺少多轮迭代优化过程，无法根据问题特征进行针对性调整。')
add_bullet(doc, '第三点：缺少剧情上下文。提示词优化时未考虑主体所属的剧集、剧本、场景情绪等上下文信息，导致优化方向与剧情需求脱节。')
add_bullet(doc, '第四点：无质量检测与自动优化闭环。生成后缺少自动质量检测环节，问题图片无法被自动识别和重试优化。')
doc.add_page_break()
...
add_bullet(doc, '用户价值：大幅提升图片生成质量与一次成功率，减少反复调整和重生成的时间成本。')
add_bullet(doc, '产品价值：建立"Hermes Agent 多轮对话提示词优化引擎加质量检测闭环"的技术壁垒。')
add_bullet(doc, '团队价值：通过隐式优化机制和用户确认机制，降低生成结果的不可控性。')
add_bullet(doc, '技术价值：探索 Hermes Agent 在视觉生成提示词优化领域的应用，为后续场景积累经验。')
doc.add_page_break()
