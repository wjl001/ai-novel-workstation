<template>
  <div class="script-optimizer-panel flex flex-col h-full bg-slate-900 text-slate-200 p-4 overflow-y-auto custom-scrollbar">
    <div class="mb-6">
      <h3 class="text-lg font-bold text-indigo-400 mb-2 flex items-center gap-2">
        <el-icon><VideoCamera /></el-icon> AI 短剧剧本优化引擎
      </h3>
      <p class="text-xs text-slate-500">专为短视频节奏打造，一键解决口语化、节奏感、连贯性与爽点痛点。</p>
    </div>

    <!-- 1. 口语化重写引擎 -->
    <div class="feature-card mb-4 bg-slate-800 p-4 rounded-lg border border-slate-700 hover:border-indigo-500/50 transition-colors">
      <div class="flex justify-between items-center mb-2">
        <div class="font-bold text-sm text-slate-300">🗣️ 口语化重写</div>
      ...
      <div v-if="settings.colloquial.enabled" class="space-y-2">
        ...
        <el-button type="primary" size="small" class="w-full mt-2 !bg-indigo-600/20 !border-indigo-500/30 !text-indigo-300 hover:!bg-indigo-600/30" @click="runOptimization('colloquial')">
          执行口语化重写
        </el-button>
      </div>
    </div>
    ...
    <!-- 2. 听觉节奏控制器 -->
    <div class="feature-card mb-4 bg-slate-800 p-4 rounded-lg border border-slate-700 hover:border-orange-500/50 transition-colors">
      <div class="flex justify-between items-center mb-2">
        <div class="font-bold text-sm text-slate-300">🎵 听觉节奏控制</div>
      ...
      <div v-if="settings.rhythm.enabled" class="space-y-2">
        ...
        <el-button type="warning" size="small" class="w-full mt-2 !bg-orange-600/20 !border-orange-500/30 !text-orange-300 hover:!bg-orange-600/30" @click="runOptimization('rhythm')">
          调整节奏与时长
        </el-button>
      </div>
    </div>
    ...
    <!-- 3. 剧情连贯性记忆链 -->
    <div class="feature-card mb-4 bg-slate-800 p-4 rounded-lg border border-slate-700 hover:border-cyan-500/50 transition-colors">
      <div class="flex justify-between items-center mb-2">
        <div class="font-bold text-sm text-slate-300">🔗 剧情连贯性记忆链</div>
      ...
      <div v-if="settings.consistency.enabled" class="space-y-2">
        ...
        <el-button type="info" size="small" class="w-full !bg-cyan-600/20 !border-cyan-500/30 !text-cyan-300 hover:!bg-cyan-600/30" @click="runOptimization('consistency')">
          检查并修复连贯性
        </el-button>
      </div>
    </div>
    ...
    <!-- 4. 爽点/钩子强化器 -->
    <div class="feature-card mb-4 bg-slate-800 p-4 rounded-lg border border-slate-700 hover:border-red-500/50 transition-colors">
      <div class="flex justify-between items-center mb-2">
        <div class="font-bold text-sm text-slate-300">🔥 爽点/钩子强化</div>
      ...
      <div v-if="settings.hooks.enabled" class="space-y-2">
        ...
        <el-button type="danger" size="small" class="w-full mt-2 !bg-red-600/20 !border-red-500/30 !text-red-300 hover:!bg-red-600/30" @click="runOptimization('hooks')">
          强化爽点与悬念
        </el-button>
      </div>
    </div>
    ...
    <div class="feature-card mb-4 bg-slate-800 p-4 rounded-lg border border-slate-700 hover:border-purple-500/50 transition-colors">
      <div class="flex justify-between items-center mb-2">
        <div class="font-bold text-sm text-slate-300">🎥 影视化对接</div>
      ...
      <div v-if="settings.videoBridge.enabled" class="space-y-2">
        ...
        <el-button type="primary" size="small" class="w-full !bg-purple-600/20 !border-purple-500/30 !text-purple-300 hover:!bg-purple-600/30" @click="runOptimization('visual_prompts')">
          提取画面提示词 (Midjourney/SD)
        </el-button>
        <el-button type="primary" size="small" class="w-full !bg-purple-600/20 !border-purple-500/30 !text-purple-300 hover:!bg-purple-600/30" @click="runOptimization('script_format')">
          生成分镜脚本表格
        </el-button>
      </div>
    </div>
    ...
    <el-dialog v-model="showResultDialog" :title="resultTitle" width="600px" class="dark-dialog" append-to-body>
      <div class="flex flex-col gap-4 h-[400px]">
         <div class="flex-1 bg-slate-900 rounded p-4 border border-slate-700 overflow-y-auto text-slate-300 whitespace-pre-wrap leading-relaxed custom-scrollbar text-sm">
            <div v-if="isProcessing" class="flex items-center justify-center h-full text-slate-500 gap-2">
      ...
      <template #footer>
        <el-button @click="showResultDialog = false" class="!bg-slate-700 !border-slate-600 !text-slate-300 hover:!text-white">取消</el-button>
        <el-button type="primary" :disabled="isProcessing" @click="applyResult" class="!bg-indigo-600 border-none">应用修改</el-button>
      </template>
    </el-dialog>
  </div>
</template>
