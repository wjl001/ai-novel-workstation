<template>
  <div class="role-permission-container">
    <div class="flex items-start gap-4 mb-8 bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-2xl border border-blue-100">
      <div class="w-12 h-12 rounded-full bg-white shadow-sm flex items-center justify-center shrink-0">
        <el-icon class="text-2xl text-blue-500"><WarningFilled /></el-icon>
      </div>
      <div>
        <h3 class="text-lg font-black text-slate-800 mb-2">角色权限说明</h3>
        <p class="text-sm text-slate-600 leading-relaxed">
          子账号的所有操作均严格受其"角色权限"控制。为确保各司其职并保障资产安全，创建子账号时需为其设定角色。<br/>
          <span class="text-indigo-600 font-medium">注：同一子账号在所有项目中的权限保持一致，由其角色决定。</span>
        </p>
      </div>
    </div>

    <div class="bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-sm mb-8">
      <el-table 
        :data="roleData" 
        style="width: 100%" 
      ...
      >
        <el-table-column prop="category" label="角色类别" width="140">
          <template #default="{ row }">
            <span class="font-bold" :class="row.category === '管理类角色' ? 'text-rose-500' : 'text-emerald-500'">
              {{ row.category }}
            </span>
          </template>
        </el-table-column>
        <el-table-column prop="roleName" label="角色名称" width="200">
          <template #default="{ row }">
            <div class="flex items-center gap-2">
              ...
              <span class="font-bold text-slate-800">{{ row.roleName }}</span>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="responsibility" label="主要职责" width="160">
          <template #default="{ row }">
            <el-tag size="small" type="info" class="!border-slate-200 !text-slate-600 !bg-slate-50">
              {{ row.responsibility }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="managementScope" label="管理范围" width="180">
          <template #default="{ row }">
            <span class="text-slate-600 font-medium">{{ row.managementScope }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="assetPermissions" label="资产及创作权限">
          <template #default="{ row }">
            <p class="text-sm text-slate-500 leading-relaxed">{{ row.assetPermissions }}</p>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow group">
        <div class="w-12 h-12 rounded-xl bg-indigo-50 flex items-center justify-center mb-4 group-hover:bg-indigo-500 transition-colors">
          <el-icon class="text-2xl text-indigo-500 group-hover:text-white transition-colors"><Connection /></el-icon>
        </div>
        <h4 class="font-bold text-slate-800 mb-2 text-lg">账号关系</h4>
      ...
      <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow group">
        <div class="w-12 h-12 rounded-xl bg-purple-50 flex items-center justify-center mb-4 group-hover:bg-purple-500 transition-colors">
          <el-icon class="text-2xl text-purple-500 group-hover:text-white transition-colors"><FolderChecked /></el-icon>
        </div>
        <h4 class="font-bold text-slate-800 mb-2 text-lg">资产归属</h4>
        <p class="text-sm text-slate-500 leading-relaxed">
      ...
      <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow group">
        <div class="w-12 h-12 rounded-xl bg-orange-50 flex items-center justify-center mb-4 group-hover:bg-orange-500 transition-colors">
          <el-icon class="text-2xl text-orange-500 group-hover:text-white transition-colors"><Coin /></el-icon>
        </div>
        <h4 class="font-bold text-slate-800 mb-2 text-lg">积分流转</h4>
        <p class="text-sm text-slate-500 leading-relaxed">
