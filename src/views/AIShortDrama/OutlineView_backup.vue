  ...
  <div class="flex h-full w-full overflow-hidden p-4 lg:p-6 gap-4 lg:gap-6 text-[#1f2329] bg-gradient-to-br from-[#F8FAFC] to-[#F1F5F9] dark:from-slate-900 dark:to-slate-800 relative transition-all duration-500" :class="{'is-left-collapsed': isLeftCollapsed, 'is-right-collapsed': !isRightPanelVisible}">
        ...
        >
          ...
          >
            ...
            <div class="flex flex-col">
              <span class="font-black text-[14px]">引用�?AI 助手</span>
              <span class="text-[10px] opacity-60 font-medium group-hover:text-white/80">以此灵感开启对�?/span>
            </div>
          </div>
        </div>
      </transition>
            ...
            <template v-else-if="form">
              ...
              <div class="flex flex-col gap-3 pb-6">
                ...
                <div v-if="!form?.episodesData || form.episodesData.length === 0" class="py-20 flex flex-col items-center justify-center bg-slate-50/50 dark:bg-slate-900/50 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-[32px] text-slate-400">
                  ...
                  <p class="text-[12px] mt-1 opacity-60">点击上方按钮开始规划你的故�?/p>
                </div>
              </div>
            </template>
          </div>
    ...
    <div v-if="form" class="right-panel flex-1 flex gap-4 lg:gap-6 h-full min-h-0 transition-all duration-500 overflow-visible">
      ...
      <div class="flex-1 flex flex-col h-full min-h-0 bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl rounded-[28px] shadow-2xl shadow-slate-200/50 dark:shadow-none border border-white dark:border-slate-700/50 overflow-hidden relative group z-10">
        ...
        <!-- Canvas Toolbar -->
        <div class="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-700/50 shrink-0 bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl">
          <div class="flex items-center gap-5">
            <div class="relative group">
              <div class="absolute inset-0 bg-gradient-to-tr from-indigo-600 to-purple-600 rounded-2xl blur-lg opacity-20 group-hover:opacity-40 transition-opacity"></div>
              <div class="relative w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-600 to-purple-600 flex items-center justify-center text-white shadow-xl rotate-3 group-hover:rotate-0 transition-transform duration-500">
            ...
            <div class="min-w-0">
              <div class="flex items-center gap-3 mb-1">
                <transition name="fade" mode="out-in">
                  <h3 :key="currentEpisodeIndex + editMode + (currentEpisode?.id || '')" class="text-[20px] font-black text-slate-800 dark:text-white tracking-tight">
                    {{ editMode === 'full' ? '全集剧本分镜�? : '分镜脚本正文' }}
                  </h3>
                </transition>
                <div class="px-2.5 py-0.5 rounded-lg bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 text-[10px] font-black border border-indigo-100 dark:border-indigo-800 uppercase tracking-widest">
                  Storyboard Mode
                </div>
              </div>
              <div class="flex items-center gap-2">
                <span class="text-[11px] text-slate-400 font-bold uppercase tracking-widest">
                  {{ editMode === 'full' ? 'Comprehensive Scripting' : `Editing: ${currentEpisodeLabel}` }}
                </span>
                <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                <div class="flex items-center gap-1">
                  <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                  <span class="text-[11px] text-emerald-600 dark:text-emerald-400 font-black uppercase tracking-widest">Live Editor</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-3">
            <transition name="fade">
              <div v-if="isSavingScript" class="flex items-center gap-2 px-4 py-2 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-800">
                <el-icon class="is-loading text-indigo-500" :size="14"><Loading /></el-icon>
                <span class="text-[12px] font-bold text-slate-500 tracking-tight">自动保存�?..</span>
              </div>
              <div v-else class="flex items-center gap-2 px-4 py-2 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-100 dark:border-emerald-900/50">
                <el-icon class="text-emerald-500" :size="14"><Check /></el-icon>
                <span class="text-[12px] font-bold text-emerald-600 dark:text-emerald-400 tracking-tight">已自动保�?/span>
              </div>
            </transition>
          </div>
        </div>
        ...
        <!-- Canvas Main Body -->
        <div class="flex-1 min-h-0 relative bg-[#FDFDFD] dark:bg-slate-900/30 flex flex-col script-canvas-container">
          ...
          <transition name="fade">
            <div v-if="isSwitchingContent" class="absolute inset-0 z-50 flex items-center justify-center bg-white/60 dark:bg-slate-900/60 backdrop-blur-sm rounded-[20px]">
              <div class="flex flex-col items-center gap-3 bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-xl border border-indigo-100 dark:border-slate-700">
                ...
                <span class="text-[13px] font-black text-indigo-600 dark:text-indigo-400 tracking-widest uppercase">Syncing Script...</span>
              </div>
            </div>
          </transition>
          ...
          >
            <div
              class="min-h-full bg-white dark:bg-slate-800/80 border border-slate-100 dark:border-slate-700/50 rounded-[32px] shadow-2xl shadow-slate-200/50 dark:shadow-none relative overflow-hidden transition-all duration-300 storyboard-paper-effect"
              :class="isSwitchingContent ? 'opacity-50 scale-[0.98]' : 'opacity-100 scale-100'"
            >
              <!-- Script Pattern Overlay -->
              <div class="absolute inset-0 pointer-events-none opacity-[0.03] dark:opacity-[0.05] script-pattern-grid"></div>
              ...
              <bubble-menu 
                ...
                class="flex items-center gap-1 p-1.5 bg-white dark:bg-slate-800 rounded-2xl shadow-[0_20px_50px_rgba(99,102,241,0.2)] border border-indigo-100 dark:border-indigo-900/50 overflow-hidden"
              >
                <button 
                  @click="quoteSelectedText"
                  class="flex items-center gap-2 px-4 py-2 text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 rounded-xl transition-all duration-300 shadow-lg shadow-indigo-500/20 group scale-95 hover:scale-100"
                >
                  <el-icon :size="16"><ChatLineSquare /></el-icon>
                  <span class="text-[13px] font-black">AI 智能解析</span>
                </button>
              </bubble-menu>

              <!-- Editor Content -->
              <editor-content v-if="tiptapEditor" :editor="tiptapEditor as any" class="p-8 lg:p-12 outline-none min-h-[500px] storyboard-prose" />

              <!-- Empty State -->
              <transition name="fade">
                <div v-if="!isGenerating && !editorTextContent && showEmptyPlaceholder && !isEpisodeLocked(currentEpisodeIndex)" class="absolute inset-0 flex flex-col items-center justify-center bg-white/95 dark:bg-slate-900/95 z-10 p-8 text-center backdrop-blur-md">
                  <div class="relative mb-8">
                    <div class="w-32 h-32 rounded-[48px] bg-gradient-to-tr from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 flex items-center justify-center text-indigo-200 dark:text-indigo-500/30 border border-white dark:border-slate-700 shadow-xl shadow-indigo-500/5 rotate-3 animate-float">
                      <el-icon size="64"><Document /></el-icon>
                    </div>
                    <div class="absolute -right-2 -bottom-2 w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-600 to-purple-600 flex items-center justify-center text-white shadow-lg shadow-indigo-500/30 -rotate-12 animate-bounce-subtle">
                      <el-icon size="24"><MagicStick /></el-icon>
                    </div>
                  </div>
                  <h4 class="text-[24px] font-black text-slate-800 dark:text-white mb-3 tracking-tight">
                    {{ editMode === 'full' ? '全集剧本灵感�? : `开�?${currentEpisodeLabel} 的创作之旅` }}
                  </h4>
                  <p class="mb-10 text-slate-500 dark:text-slate-400 text-[14px] font-medium max-w-sm leading-relaxed">
                    {{ editMode === 'full' ? '不要让空白页阻碍你的创意。点击下方按钮，�?AI 帮你搭建故事的骨架�? : `当前剧集内容还是空白，你可以手动输入分镜脚本，或者让 AI 智能生成。` }}
                  </p>
                  <div class="flex items-center gap-6">
                    <button 
                      @click="generateScriptBody"
                      class="h-[52px] px-8 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-[20px] text-[15px] font-black shadow-xl shadow-indigo-500/30 hover:scale-105 active:scale-95 transition-all flex items-center gap-2 group"
                    >
                      <el-icon :size="20" class="group-hover:rotate-12 transition-transform"><MagicStick /></el-icon> 
                      {{ editMode === 'full' ? '一键生成全集脚�? : `生成${currentEpisodeLabel}分镜脚本` }}
                    </button>
                    <button 
                      @click="startManualWrite"
                      class="h-[52px] px-8 bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 rounded-[20px] text-[15px] font-black hover:bg-slate-50 dark:hover:bg-slate-800 transition-all flex items-center gap-2 shadow-sm"
                    >
                      <el-icon :size="20"><Edit /></el-icon> 灵感突现，手写创�?                    </button>
                  </div>
                </div>
              </transition>
        ...
        <!-- Tiptap Footer Stats -->
        <div class="h-10 border-t border-slate-100 dark:border-slate-700/50 flex items-center justify-between px-8 text-[11px] font-black text-slate-500 bg-white dark:bg-slate-800 shrink-0">
          <div class="flex items-center gap-8 uppercase tracking-widest">
            <span class="flex items-center gap-2 group cursor-default">
              ...
              字数统计: <span class="text-indigo-600 dark:text-indigo-400">{{ tiptapEditor?.storage.characterCount.characters() || 0 }}</span>
            </span>
            <span class="flex items-center gap-2 group cursor-default">
              <span class="w-2 h-2 rounded-full bg-purple-500 group-hover:scale-125 transition-transform"></span> 
              预计时长: <span class="text-purple-600 dark:text-purple-400">{{ Math.ceil((tiptapEditor?.storage.characterCount.words() || 0) / 250) }} 分钟</span>
            </span>
          </div>
          <div class="flex items-center gap-4">
            <button @click="tiptapEditor?.commands.undo()" class="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-xl transition-all border border-transparent hover:border-indigo-100 dark:hover:border-indigo-800">
              <el-icon :size="14"><RefreshLeft /></el-icon>
            </button>
            <button @click="tiptapEditor?.commands.redo()" class="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-xl transition-all border border-transparent hover:border-indigo-100 dark:hover:border-indigo-800">
        ...
        <!-- Action Footer -->
        <div class="flex justify-end items-center p-6 border-t border-slate-100 dark:border-slate-700/50 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md shrink-0 gap-5">
          <div v-if="isGenerating" class="flex items-center gap-4 text-indigo-600 dark:text-indigo-400 text-[13px] mr-auto bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 px-5 py-2.5 rounded-2xl font-black border border-indigo-100 dark:border-indigo-900/50 shadow-inner">
            <el-icon class="is-loading" v-if="!isPaused" :size="16"><Loading /></el-icon> 
            <span class="tracking-wide">{{ isPaused ? '生成已暂�? : 'AI 正在构思精妙情�?..' }}</span>
            <button 
              @click="isPaused = !isPaused" 
              class="flex items-center gap-1.5 px-4 py-1.5 bg-white dark:bg-slate-800 rounded-xl text-indigo-600 shadow-md hover:scale-105 active:scale-95 transition-all border border-indigo-50 dark:border-slate-700"
            >
              <el-icon :size="14"><VideoPause v-if="!isPaused"/><VideoPlay v-else/></el-icon> 
              <span class="text-[11px]">{{ isPaused ? '恢复生成' : '暂停' }}</span>
            </button>
          </div>
          ...
          <button 
            v-if="!isGenerating" 
            @click="generateScriptBody"
            class="h-12 px-7 bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 border-2 border-indigo-100 dark:border-indigo-900/50 rounded-[20px] text-[14px] font-black hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:border-indigo-200 transition-all flex items-center gap-2.5 shadow-sm hover:shadow-md"
          >
            <el-icon v-if="!editorTextContent" :size="18"><MagicStick /></el-icon>
            <el-icon v-else :size="18"><Refresh /></el-icon>
            {{ editorTextContent ? (editMode === 'full' ? '不满意？重写全集' : `重写${currentEpisodeLabel}`) : (editMode === 'full' ? '帮我写全�? : `帮我�?{currentEpisodeLabel}`) }}
          </button>

          <button 
            @click="goToSubjectSettings"
            :disabled="isSavingScript || !editorTextContent"
            class="h-12 px-10 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500 text-white rounded-[20px] text-[14px] font-black shadow-xl shadow-indigo-500/30 hover:scale-[1.03] hover:shadow-indigo-500/40 active:scale-95 disabled:opacity-50 disabled:pointer-events-none transition-all flex items-center gap-2.5 group"
          >
            <el-icon v-if="isSavingScript" class="is-loading" :size="18"><Loading /></el-icon>
            <span>下一步：设置主体</span>
            <el-icon v-if="!isSavingScript" class="group-hover:translate-x-1 transition-transform" :size="18"><ArrowRight /></el-icon>
          </button>
        </div>
      </div>
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-6 p-6" ref="rightPanelChatRef">
              ...
              <div class="flex flex-col gap-2 items-start animate-slide-up">
                ...
                <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl rounded-tl-none shadow-sm text-[13px] leading-relaxed text-slate-700 dark:text-slate-200 border border-slate-100 dark:border-slate-700/50 max-w-[90%]">
                  你好！我是你的专�?AI 创作助手。我已经准备好帮你打磨剧本、丰富情节或者提供灵感了�?                </div>
              </div>
              ...
              <transition-group name="chat">
                ...
                <div v-if="aiProposal.isActive" key="ai-proposal" class="flex flex-col gap-3 animate-slide-up">
                  ...
                  <div class="bg-white dark:bg-slate-800 rounded-[20px] shadow-xl border border-indigo-100 dark:border-indigo-900/50 overflow-hidden">
                    ...
                    <div v-if="!aiProposal.isGenerating" class="p-4 bg-slate-50 dark:bg-slate-900/50 flex items-center gap-3">
                      ...
                      >
                        <el-icon><Check /></el-icon> 采纳并应�?                      </button>
                    </div>
                  </div>
                </div>
                ...
                >
                  <div class="flex items-center gap-2 px-1" :class="msg.role === 'ai' ? 'flex-row' : 'flex-row-reverse'">
                    ...
                    <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ msg.role === 'ai' ? 'AI 助手' : '�? }}</span>
                  </div>
                  <div 
        ...
        <div class="prose dark:prose-invert max-w-none">
          ...
          <ul class="space-y-3 mb-6">
            <li class="flex items-start gap-2 bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-50 dark:border-slate-700/50">
              ...
              <span class="text-slate-600 dark:text-slate-300"><strong>左侧 (设定与大�?�?/strong>基础设定（背景、梗概）与剧集分集大纲（拖拽排序、剧情摘要）�?/span>
            </li>
            <li class="flex items-start gap-2 bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-50 dark:border-slate-700/50">
              <span class="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-2 shrink-0"></span>
              <span class="text-slate-600 dark:text-slate-300"><strong>中栏 (剧本编辑�?�?/strong>采用好莱坞标准剧本格式排版的富文本编辑器�?/span>
            </li>
            <li class="flex items-start gap-2 bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-50 dark:border-slate-700/50">
              <span class="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-2 shrink-0"></span>
...
const isEpisodeLocked = (index: number) => {
  ...
  // 检查前一集是否有实际内容（过滤掉空标签、分隔符、空格等�?  const content = prevEpisode?.content || '';
...
const currentEpisodeLabel = computed(() => {
  const fallback = `�?${currentEpisodeIndex.value + 1} 集`;
...
const selectEpisode = async (index: number | string) => {
  if (isSwitchingEpisode) return; // 防止快速点击导致状态错�?
  try {
    ...
    // 如果已经在该集且是分集模式，则不需要切�?    if (editMode.value === 'episode' && currentEpisodeIndex.value === targetIndex) return;

    // 触发切换淡出动画
    isSwitchingContent.value = true;
    
    // 等待 150ms 以允�?fade-out 动画执行
    ...
    // 更新状�?    editMode.value = 'episode';
    currentEpisodeIndex.value = targetIndex;
    isRightPanelVisible.value = true;
    
    // 加载目标分集的内�?    loadContentFromStore();

    // 加载目标分集的聊天历�?    if (form.value.episodesData[targetIndex]) {
    ...
    try {
      ...
      console.error('nextTick 调用失败:', error);
    } finally {
      // 无论成功或失败，继续执行后续逻辑
    }
...
const saveCurrentContentToStore = () => {
  try {
    ...
    if (editMode.value === 'full') {
      ...
      // 使用正则匹配分隔符，允许标签内包含样式属�?      const parts = content.split(/<p[^>]*>------------------<\/p>/g);
      ...
      // 同步全集内容：重新合并所有分�?      form.value.fullContent = form.value.episodesData
...
watch(chatMessages, (newMsgs) => {
  if (form.value && editMode.value === 'episode' && form.value.episodesData && form.value.episodesData[currentEpisodeIndex.value]) {
    // 避免在此处改变引用的地址，否则会触发上方�?deep watch 造成循环
...
const goToSubjectSettings = async () => {
  if (!editorTextContent.value) {
    ElMessage.warning('请先生成或编写剧本内容，再进行后续设�?);
    return;
  }
  isSavingScript.value = true;
  // 强制同步当前所有内�?  saveCurrentContentToStore();
...
const acceptAIProposal = () => {
  if (tiptapEditor.value && aiProposal.isActive) {
    ...
    ElMessage.success('已应�?AI 建议到剧�?);
  }
};

const rejectAIProposal = () => {
  aiProposal.isActive = false;
  ElMessage.info('已放�?AI 建议');
};
...
onMounted(async () => {
  ...
  tiptapEditor.value = new Editor({
    extensions: [
      ...
      Placeholder.configure({
        placeholder: ({ node }) => {
          return editMode.value === 'full' ? '全集剧本还是一片空�? : currentEpisodeLabel.value;
        }
      })
    ],
    content: '',
    onUpdate: ({ editor }) => {
      // 防止在切换内容时产生�?update 触发无限循环
  ...
  // 监听锁定状态、当前分集、编辑模式或分集内容变化，实时更新编辑器可编辑状�?  watch([isEditingLocked, currentEpisodeIndex, editMode, () => form.value?.episodesData], () => {
    // 防止在编辑器加载时频繁切换状态触发循�?    if (isSwitchingContent.value) return;
...
.storyboard-prose p:hover {
  background: rgba(99, 102, 241, 0.02);
  border-color: rgba(99, 102, 241, 0.1);
}

/* Scene Header Style: starts with "第x�? or "场景" */
.storyboard-prose p:has(strong):first-child,
.storyboard-prose p:contains("场景"),
.storyboard-prose p:contains("�?) {
...
/* Action/Visual Style: starts with "�? or "画面" */
.storyboard-prose p:contains("�?),
.storyboard-prose p:contains("画面"),
.storyboard-prose p:contains("动作") {
...
.storyboard-prose p:contains("�?)::before {
  content: 'SHOT';
  position: absolute;
  left: -2.5rem;
  top: 50%;
...
/* Assuming format "Character Name: Dialogue" */
.storyboard-prose p:contains("�?),
.storyboard-prose p:contains(":") {
  padding-left: 4rem;
  position: relative;
...
.storyboard-prose p:contains("�?) strong,
.storyboard-prose p:contains(":") strong {
...
/* Placeholder */
.storyboard-prose p.is-editor-empty:first-child::before {
  color: #94a3b8;
...
.dark .storyboard-prose {
  color: #cbd5e1;
}

.dark .storyboard-prose p:hover {
  background: rgba(99, 102, 241, 0.05);
}

.dark .storyboard-prose p:contains("场景") {
  background: linear-gradient(to right, rgba(99, 102, 241, 0.2), transparent);
  color: #818cf8;
}

/* Prose Modern Styling (Keep for fallback or other areas if needed, but updated to storyboard) */
.prose-modern {
  @apply storyboard-prose;
}
